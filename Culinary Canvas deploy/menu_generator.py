...
import os
# from secret_key import API_KEY as api_key # Removed, API key is passed dynamically
...
""",
    ...
    def generate_menu(
        self, cuisine: str, diets: Union[str, List[str]], no_of_items: int = 3
    ) -> MenuResponse:
        """
        ...
        try:
            ...
            # Create chains
            name_chain = LLMChain(
                llm=self.llm, prompt=name_template, output_key="restaurant_name"
            )
...
if __name__ == "__main__":
    ...
    if api_key:
        cuisine = st.selectbox("Select Cuisine:", ["Mexican", "Italian", "Indian"])
        diets = st.multiselect("Select Dietary Restrictions:", ["vegetarian", "vegan", "gluten-free"])
        no_of_items = st.slider("Number of Items per Section:", 1, 5, 3)

        if st.button("Generate Menu"):
            try:
                generator = RestaurantMenuGenerator(key=api_key)
                response = generator.generate_menu(
                    cuisine=cuisine, diets=diets, no_of_items=no_of_items
                )

                st.subheader(f"Restaurant Name: {response.restaurant_name}")
        ...
        st.warning("Please enter your API key to use the generator.")